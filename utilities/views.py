...
def list_view(request, model_name, app_name,html_name='',field_names = '',
    ...
    var = {name +'_list':instances,'page_name':model_name,
        ...
        'type_name':name+'_type','app_name':app_name,
...
def edit_model(request, name_space, model_name, app_name,  
    instance_id = None, formset_names='', focus='', view ='complete'):
    '''edit view generalized over models.
    assumes a 'add_{{model_name}}.html template 
    and edit_{{model_name}} function
    and {{model_name}}Form
    '''
    print('focus',[focus])
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            print(nfe)
            [show_messages(request,'error',l) for l in nfe]
    ...
    if instance_id: page_name = 'Edit ' + model_name.lower()
    else: page_name = 'Add ' + model_name.lower()
    helper = help_util.Helper(model_name = model_name)
    args = {'form':form,'page_name':page_name,'crud':crud,
        'tabs':tabs, 'view':view,'app_name':app_name,
        'model_name':model_name,
        'helper':helper.get_dict(), 'thumbnail_size':thumbnail_size,
        'focus':focus}
    args.update(ffm.dict)
    url = app_name+'/add_' + model_name.lower() + '.html'
    return render(request,url,args)
        

@permission_required('utilities.add_generic')
def add_simple_model(request, name_space,model_name,app_name, 
    page_name, pk= None):
    ...
    if pk: page_name = 'Edit ' +model_name.lower()
    else: page_name = 'Add ' +model_name.lower()
...
def edit_protocol(request, app_name, model_name, field_name = None):
    ...
    queryset=Protocol.objects.filter(app_name = app_name,
        model_name=model_name)
    formset = []
    if request.method == 'POST':
        ...
        if formset.is_valid():
            ...
            for instance in instances:
                need_save = False
                if not instance.app_name or instance.model_name: 
                    need_save = True
    ...
    note = 'changes to the protocol text will be visible after '
