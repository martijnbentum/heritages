...
This can blow up the size of the database, especially with programatic changes to instances.

CRUD events without changes (saving an instance without changing anything
or wihtout a user (programmatic changes) are delete to save disk space
'''
...
class Command(BaseCommand):
	...
	def handle(self, *args, **options):
		start = time.time()
		print('cleaning database by removing superious CRUD events and VACUUM the database')
		...
		print('removing',len(o),'spurious CRUD events (without user or changed fields)')
		for event in o:
			event.delete()
